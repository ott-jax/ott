...
class TestSoftSort:
  ...
  def test_multivariate_cdf_quantiles(self, rng: jax.Array):
    ...
    z = jr.uniform(key1, (1, d))

    # Sample inputs symmetrically centered on z
    inputs = 0.34 * jr.normal(key2, (n, d)) + z
  ...
  def test_ranks(self, axis: int, rng: jax.Array, jit: bool):
    ...
    target_weights = jr.uniform(rng2, (num_targets,))
  ...
  def test_quantile_on_several_axes(self, rng: jax.Array):
    ...
    x = jr.uniform(rng, shape=(batch, height, width, channels))
  ...
  def test_sort_with(self, rng: jax.Array):
    n, d = 20, 4
    inputs = jr.uniform(rng, shape=(n, d))
  ...
  def test_soft_sort_jacobian(self, rng: jax.Array, implicit: bool):
    ...
    delta = jr.uniform(rngs[2], z.shape) - 0.5
    eps = 1e-3
