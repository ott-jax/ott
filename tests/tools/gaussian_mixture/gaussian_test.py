...
class TestGaussian:
  ...
  def test_from_random(self, rng: jnp.ndarray):
    g = gaussian.Gaussian.from_random(key=rng, n_dimensions=3)
  ...
  def test_to_z(self, rng: jnp.ndarray):
    g = gaussian.Gaussian(
        loc=jnp.array([1., 2.]),
    ...
    samples = g.sample(key=rng, size=1000)
    z = g.to_z(samples)
  ...
  def test_from_z(self, rng: jnp.ndarray):
    g = gaussian.Gaussian(
        loc=jnp.array([0., 0.]),
        scale=scale_tril.ScaleTriL(
  ...
  def test_sample(self, rng: jnp.ndarray):
    ...
    samples = g.sample(key=rng, size=10000)
