...
class TestLinalg:
  ...
  def test_get_mean_and_cov(self, rng: jax.random.PRNGKeyArray):
    points = jax.random.normal(key=rng, shape=(10, 2))
    weights = jnp.ones(10)
    expected_mean = jnp.mean(points, axis=0)
  ...
  def test_flat_to_tril(self, rng: jax.random.PRNGKeyArray):
    size = 3
  ...
  def test_tril_to_flat(self, rng: jax.random.PRNGKeyArray):
    size = 3
  ...
  def test_apply_to_diag(self, rng: jax.random.PRNGKeyArray):
    size = 3
    m = jax.random.normal(key=rng, shape=(5, 4, size, size))
  ...
  def test_matrix_powers(self, rng: jax.random.PRNGKeyArray):
  ...
  def test_invmatvectril(self, rng: jax.random.PRNGKeyArray):
    rng, subrng = jax.random.split(rng)
  ...
  def test_get_random_orthogonal(self, rng: jax.random.PRNGKeyArray):
    rng, subrng = jax.random.split(rng)
