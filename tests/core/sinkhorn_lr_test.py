...
class SinkhornLRTest(parameterized.TestCase):
  ...
  # @parameterized.product(use_lrcgeom=[True, False], init_type=['random', 'rank_2'])
  ...
  def test_euclidean_point_cloud(self, use_lrcgeom):
    ...
    for init_type in init_type_arr:
        ...
        solver = sinkhorn_lr.LRSinkhorn(threshold=threshold, gamma=gamma, rank=1, epsilon=0.0, init_type=init_type)
        ...
        solver = sinkhorn_lr.LRSinkhorn(threshold=threshold, gamma=gamma, rank=10, epsilon=1e-3, init_type=init_type)
        out = solver(ot_prob)
        costs = out.costs
