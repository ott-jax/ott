...
class BidirectionalPotentials(DualPotentials):
    ...
    def __init__(
        self,
        g: PotentialValueFn_t,
        grad_f: PotentialGradientFn_t,
...
class EuclideanPotentials(DualPotentials):
    ...
    def distance(self, src: jnp.ndarray, tgt: jnp.ndarray) -> float:
        ...
        out = -2.0 * corr + jnp.mean(jnp.sum(src ** 2, axis=-1)) + jnp.mean(jnp.sum(tgt ** 2, axis=-1))
        return out


def expectile_loss(adv: jnp.ndarray, diff: jnp.ndarray, expectile: float=0.9) -> jnp.ndarray:
...
class PotentialTrainState(train_state.TrainState):
    ...
    potential_value_fn: Callable[
        [frozen_dict.FrozenDict[str, jnp.ndarray]], PotentialValueFn_t
    ] = struct.field(pytree_node=False)
    
    potential_gradient_fn: Callable[
        [frozen_dict.FrozenDict[str, jnp.ndarray]], PotentialGradientFn_t
    ] = struct.field(pytree_node=False)
...
class ExpectileNeuralDual(Generic[CostFn_t]):
    ...
    r"""Expectile-regularized Neural Optimal Transport (ENOT) method arXiv:2403.03777. 
    ...
    and :math:`g`. The main optimisation objective is

    .. math::

       \sup_{g \in L_1(\beta)} \inf_{T: \, R^d \to R^d} \Bigr[\mathbb{E}_{\alpha}[ c(\xx, T(x)) ]  +   
       \mathbb{E}_{\beta} [g (\yy)] - \mathbb{E}_{\alpha} [g(T(x)) ]\Bigr],
    ...
    def __init__(
        ...
        assert (
            not is_bidirectional or isinstance(cost_fn, costs.TICost) or isinstance(cost_fn, DotCost)
        ...
        if neural_f is None:
            ...
            neural_f = MLP(dim_hidden=[128, 128, 128, 128, last_dim], act_fn=jax.nn.elu)
        if neural_g is None:
            neural_g = MLP(dim_hidden=[128, 128, 128, 128, 1], act_fn=jax.nn.elu)

        self.neural_f = PotentialModelWrapper(model=neural_f, is_potential=is_bidirectional, add_l2_norm=self.use_dot_product)
        self.neural_g = PotentialModelWrapper(model=neural_g, is_potential=True, add_l2_norm=self.use_dot_product)

        rng = utils.default_prng_key(rng)
        rng, rng_f, rng_g = jax.random.split(rng, 3)

        self.state_f = self.neural_f.create_train_state(rng_f, optimizer_f, (1, dim_data))
        self.state_g = self.neural_g.create_train_state(rng_g, optimizer_g, (1, dim_data))
    ...
    def train_fn(
        ...
        for step in tqdm(range(self.num_train_iters)):
            ...
            if update_forward:
                ...
                (self.state_f, self.state_g, loss, loss_f, loss_g, w_dist) = self.train_step(self.state_f, self.state_g, train_batch)
            else:
                train_batch["target"] = jnp.asarray(next(trainloader_source))
                train_batch["source"] = jnp.asarray(next(trainloader_target))
                (self.state_g, self.state_f, loss, loss_f, loss_g, w_dist) = self.train_step(self.state_g, self.state_f, train_batch)
            ...
            if step != 0 and step % self.valid_freq == 0:
                ...
                valid_loss_f, valid_loss_g, valid_w_dist = self.valid_step(self.state_f, self.state_g, valid_batch)

                if self.logging:
                    self._update_logs(valid_logs, valid_loss_f, valid_loss_g, valid_w_dist)

        return {"train_logs": train_logs, "valid_logs": valid_logs}
    
    def get_train_step(self) -> Callable[
        [PotentialTrainState, PotentialTrainState, Dict[str, jnp.ndarray]],
        Tuple[PotentialTrainState, PotentialTrainState, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]
    ]:
    ...
    def get_valid_step(self) -> Callable[
        [PotentialTrainState, PotentialTrainState, Dict[str, jnp.ndarray]],
    ...
    def loss_fn(
            self, 
            params_f: frozen_dict.FrozenDict[str, jnp.ndarray], 
            params_g: frozen_dict.FrozenDict[str, jnp.ndarray], 
            gradient_f: Callable[[frozen_dict.FrozenDict[str, jnp.ndarray]], PotentialGradientFn_t], 
        ...
        g_star_source = batch_cost(source, target_hat_detach) - g_value_partial(target_hat_detach)
    
        diff_1 = jax.lax.stop_gradient(-batch_cost(source, target) + g_c_transform(source, target_hat_detach)) + g_target
        reg_loss_1 = expectile_loss(diff_1, diff_1, self.expectile).mean() 

        diff_2 = jax.lax.stop_gradient(-g_c_transform(source, target)) + g_star_source
    ...
    def to_dual_potentials(self) -> DualPotentials:
        ...
        if self.use_dot_product:
            ...
            return BidirectionalPotentials(g_value_partial, f_grad_partial, self.cost_fn)
        else:
