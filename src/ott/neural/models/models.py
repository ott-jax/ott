...
class ICNN(neuraldual.BaseW2NeuralDual):
    ...
    def setup(self) -> None:  # noqa: D102
        ...
        if self.gaussian_map_samples is not None:
            factor, mean = self._compute_gaussian_map_params(self.gaussian_map_samples)
        ...
        else:  # positive definite potential (the identity mapping or linear OT)
            self.pos_def_potential = layers.PosDefPotentials(
                rank=self.rank,
                num_potentials=1,
    ...
    @staticmethod
    def _compute_gaussian_map_params(samples: Tuple[jnp.ndarray, jnp.ndarray]) -> Tuple[jnp.ndarray, jnp.ndarray]:
...
class MetaInitializer(lin_init.DefaultInitializer):
    ...
    **same** geometry ``geom`` is used throughout, both for training and
    ...
    def __init__(
        ...
        opt: Optional[optax.GradientTransformation] = optax.adam(learning_rate=1e-3),  # noqa: B008
        ...
        if state is None:
            ...
            params = self.meta_model.init(self.rng, a_placeholder, b_placeholder)["params"]
            self.state = train_state.TrainState.create(apply_fn=self.meta_model.apply, params=params, tx=opt)
        else:
            self.state = state
    ...
    def init_dual_a(  # noqa: D102
        ...
        if vmap_a_val is not None or vmap_b_val is not None:
            compute_f_maybe_batch = jax.vmap(self._compute_f, in_axes=(vmap_a_val, vmap_b_val, None))
        else:
    ...
    def _get_update_fn(self):
        ...
        def dual_obj_loss_single(params, a, b):
            ...
            g_pred = self.geom.update_potential(f_pred, jnp.zeros_like(b), jnp.log(b), 0, axis=0)
            g_pred = jnp.where(jnp.isfinite(g_pred), g_pred, 0.0)

            ot_prob = linear_problem.LinearProblem(geom=self.geom, a=a, b=b)
            dual_obj = sinkhorn.compute_kl_reg_cost(f_pred, g_pred, ot_prob, lse_mode=True)
            loss = -dual_obj
            return loss, f_pred
    ...
    def _compute_f(
        self, a: jnp.ndarray, b: jnp.ndarray, params: frozen_dict.FrozenDict[str, jnp.ndarray]
    ) -> jnp.ndarray:
    ...
    def tree_flatten(self) -> Tuple[Sequence[Any], Dict[str, Any]]:  # noqa: D102
        return [self.geom, self.meta_model, self.opt], {"rng": self.rng, "state": self.state}
