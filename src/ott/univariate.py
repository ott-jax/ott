...
class UnivariateSolver:
  ...
  def __call__(
    ...
    return_transp_variables = return_transport or return_dual_vectors

    assert not(self.num_subsamples and return_transp_variables), \
      "Cannot return any transport output variables when subsampling."
    ...
    if self.quantiles is not None:
    ...
    elif is_uniform_same_size and not return_dual_vectors:
      return_transport = return_transport and not self.num_subsamples
      out = uniform_distance(x, y, geom.cost_fn, return_transport)
    else:
      if False:
        # perturb uniform inputs with small deterministic noise.
        perturb_a = jax.random.uniform(
            jax.random.PRNGKey(0), (geom.shape[0],)
        ) * 1e-7
        perturb_b = jax.random.uniform(
            jax.random.PRNGKey(1), (geom.shape[1],)
        ) * 1e-7
        perturb_a -= jnp.mean(perturb_a)
        perturb_b -= jnp.mean(perturb_b)
        a = prob.a + perturb_a
        b = prob.b + perturb_b
      else:
        a, b = prob.a, prob.b
      ...
      fn = lambda x, y, *args: quantile_distance(x[:, 0], y[:, 0], *args)
      out = fn(x, y, geom.cost_fn, a, b, return_transport, return_dual_vectors)
...
def quantile_distance(
  ...
  if return_dual_vectors:
    ...
    cliff = (jnp.abs(jnp.diff(orig_i)) > 0) & (jnp.abs(jnp.diff(orig_j)) > 0)

    #virtual_pairs = jnp.stack((orig_i[:-1], orig_j[1:])) * (2*cliff-1)
    virtual_pairs = (1 + jnp.stack((orig_i[:-1], orig_j[1:]))) * (2 * cliff - 1)

    #actual_pairs = jnp.stack((orig_i[diff_q>0], orig_j[diff_q > 0]))
    diff_q_sorted, diff_q_idx = mu.sort_and_argsort(diff_q, argsort=True)
    actual_pairs = jnp.stack((orig_i, orig_j))
    actual_pairs = (1 + actual_pairs[:, diff_q_idx]
                   ) * (2 * (diff_q_sorted > 0) - 1)
    # diff_q_idx = jnp.argsort(diff_q[diff_q>0])
    # print('actual_pairs', actual_pairs)
    # if actual_pairs.shape[1] > n+m-1:
    #   actual_pairs = actual_pairs[:,diff_q_idx][:,-n-m+1:]

    # print('actual_pairs2', actual_pairs)

    # if virtual_pairs.shape[1] == 0:
    #   pairs = actual_pairs
    # else:
    pairs = jnp.concatenate((actual_pairs, virtual_pairs), axis=1)

    idx = jnp.argsort(
        jnp.sum(pairs, axis=0) *
        jnp.concatenate((diff_q_sorted, jnp.ones((virtual_pairs.shape[1],)))),
        stable=True
    )[-n - m + 1:]
    pairs = pairs[:, idx] - 1

    cost_vector = jax.vmap(cost_fn.h)(x_[pairs[0]] - y_[pairs[1]])

    def kkt(dual_ab):
      """Eq. 3.6 in :cite:`peyre:19`, with centering constraint on dual_a."""
      dual_a, dual_b = dual_ab[:n], dual_ab[n:]
      return jnp.concatenate((
          jnp.array(jnp.sum(dual_a, keepdims=True)),
          dual_a[pairs[0, :]] + dual_b[pairs[1, :]]
      ))

    z = jnp.concatenate((jnp.zeros((1,)), cost_vector))
    operator = lx.FunctionLinearOperator(kkt, z)
    solver = lx.NormalCG(rtol=1e-4, atol=1e-4, max_steps=1000)
    sol = lx.linear_solve(operator, z, solver)
    sol = sol.value
